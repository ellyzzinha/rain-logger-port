...
function appleImg(surrogate: string, size: number): React.ReactElement | null {
  ...
  const RN = findByProps("Image", "View") ?? findByProps("Image") ?? (typeof global !== "undefined" && (global as any).ReactNative) ?? null;
...
function tryPatch(mod: Record<string, any> | null | undefined, key: string, handler: (args: any[], orig: (...a: any[]) => any) => any) {
...
function init() {
  // 1. NativeEmoji
  const emojiMod = findByProps("NativeEmoji") ?? findByProps("EmojiComponent") ?? findByProps("renderNativeEmoji");
  if (emojiMod) {
    const key = "NativeEmoji" in emojiMod ? "NativeEmoji" : "EmojiComponent" in emojiMod ? "EmojiComponent" : "renderNativeEmoji";
  ...
  const nodeMod = findByProps("renderEmoji") ?? findByProps("Emoji", "EmojiText") ?? findByProps("Emoji");
  if (nodeMod) {
    const key = Object.keys(nodeMod).find((k) => typeof nodeMod[k] === "function" && /^(emoji|Emoji|renderEmoji)/i.test(k));
    if (key) {
      tryPatch(nodeMod, key, ([props], orig) => {
        const surrogates = props?.node?.surrogate ?? props?.surrogates ?? props?.emoji?.surrogates;
        if (!surrogates) return orig(props);
  ...
  // 3. Reações
  const reactionMod = findByProps("EmojiReaction") ?? findByProps("ReactionEmoji") ?? findByProps("MessageReaction");
  if (reactionMod) {
  ...
  // 4. Picker
  const pickerMod = findByProps("EmojiPickerCell") ?? findByProps("EmojiPickerListRow") ?? findByProps("emojiPickerCell");
  if (pickerMod) {
