...
export default function StatusIcon({ platform, color, iconSize = 16 }: {
    platform: string;
    color?: string;
    iconSize?: number;
}) {
    ...
    // SVG path available: render crisp vector icon
    if (Svg && Path && path) {
        return (
            <View>
    ...
    // SVG not available: fallback colored circle
    return (
        <View style={{
            width: iconSize,
            height: iconSize,
            borderRadius: iconSize / 2,
            backgroundColor: color ?? "#80848e",
        }} />
    );
}
